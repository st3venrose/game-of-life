...
export class GameStateStoreService implements OnDestroy {
  ...
  constructor(private gameApiService: GameApiService, private localStorageService: LocalStorageService) {
  ...
  startNewGame(): Observable<IGameState> {
    ...
    return this.gameApiService
      .startGame(gameState.rows)
      .pipe(tap((gameState) => this.updateGameStateModel(gameState)));
  }
  ...
  getNewGameState(id?: number): Observable<IGameState> {
    ...
    return this.gameApiService
      .fetchNewGameState(tableId)
      .pipe(tap((gameState) => this.updateGameStateModel(gameState)));
  }

  getGameState(id: number): Observable<IGameState> {
    return this.gameApiService.fetchGameState(id).pipe(tap((gameState) => this.updateGameStateModel(gameState)));
  }
}
